...
export function MemberCard({ tier, points, memberSince = "Jan 2026", cardNumber = "4532 8976 1234" }: MemberCardProps) {
  ...
  return (
    <motion.div
      ...
      className="relative w-full aspect-[1.586/1] rounded-3xl overflow-hidden shadow-2xl cursor-pointer z-50 pointer-events-auto group"
    ...
    >
      ...
      <div className="relative h-full p-6 flex flex-col justify-between">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-white/20 backdrop-blur-sm rounded-xl flex items-center justify-center">
              <img src={logo} alt="PICPIC" className="w-10 h-10 rounded-lg object-cover" />
            </div>
            <div>
              <p className={`${config.textColor} font-bold text-sm opacity-90`}>PICPIC</p>
              <p className={`${config.textColor} text-xs opacity-70`}>Member Card</p>
            </div>
          </div>
          
          <div className="flex flex-col items-end gap-3 translate-y-[-4px]">
            <div className={`bg-gradient-to-r ${config.accentGradient} px-4 py-2 rounded-full flex items-center gap-2 shadow-lg`}>
              <Icon size={16} className={config.textColor} />
              <span className={`${config.textColor} font-bold text-sm`}>{config.name.split(" ")[0]}</span>
            </div>
            
            {/* QR Code Section (Absolute Positioning to avoid pushing content) */}
            <div className="opacity-40 group-hover:opacity-100 transition-all duration-500 transform group-hover:scale-110">
              <div className="p-1 bg-white/10 backdrop-blur-md rounded-lg border border-white/20 shadow-lg group-hover:bg-white/20 transition-all">
                <svg className={`h-10 w-10 fill-current ${config.textColor}`} viewBox="0 0 24 24">
        ...
        <div className="py-2">
          <p className={`${config.textColor} text-xs opacity-80 mb-1 font-medium`}>Total Points</p>
          <div className="flex items-baseline gap-2">
            <span className={`${config.textColor} font-black text-5xl tracking-tight`}>
              {points.toLocaleString("id-ID")}
            </span>
            <span className={`${config.textColor} text-lg opacity-80`}>pts</span>
          </div>
        </div>
        ...
        <div className="flex items-end justify-between">
          <div>
            <p className={`${config.textColor} text-xs opacity-70 mb-1`}>Card Number</p>
            <p className={`${config.textColor} font-mono text-sm font-semibold tracking-wider`}>
              {cardNumber}
            </p>
          </div>
          <div className="text-right">
            <p className={`${config.textColor} text-xs opacity-70 mb-1`}>Member Since</p>
            <p className={`${config.textColor} font-semibold text-sm`}>{memberSince}</p>
          </div>
        </div>
      </div>
